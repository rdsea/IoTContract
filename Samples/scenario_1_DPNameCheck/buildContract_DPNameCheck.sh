...
curl -v -d @"../contracts/term_DPNameCheck.json" -H "Content-Type: application/json" -X POST http://localhost:8080/salsa-engine/rest/elise/extracdg/contractterm
curl -v -d @"../contracts/contracttemplate_DPNameCheck.json" -H "Content-Type: application/json" -X POST http://localhost:8080/salsa-engine/rest/elise/extracdg/contracttemplate
